...
export async function POST(request: NextRequest) {
  try {
    ...
    const storageOption = searchParams.get('storage') || StorageProvider.getDefaultStorageType()
    ...
    try {
      uploadResult = await StorageProvider.upload({
        storageType: storageOption as any,
        documentType,
        fileName: file.name,
        fileBuffer,
        fileType: file.type,
    ...
    const { data: document, error: docError } = await supabase
      ...
      .insert([{
        ...
        mega_file_id: uploadResult.mega_file_id || null,
